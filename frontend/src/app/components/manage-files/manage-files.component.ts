...
export class ManageFilesComponent implements OnInit {
  ...
  registerFile() {
    ...
    if (this.filesEventReceived) {
      this.httpLessonService.addFile(this.lesson.id, this.idOfMetadataFile).subscribe((res: any) => {
  ...
  getAllFiles() {
    this.httpLessonService.getAllFiles(this.lesson.id).subscribe((files: any) => {
    this.files = files;
    });
  }
  ...
  removeFile(idFile) {
    this.httpLessonService.removeFile(this.idLesson, idFile).subscribe((res: any) => {
        console.log(res);
        this.getAllFiles();
    });
  }
  ...
  downloadFile(idFile) {
    return this.httpFileService.getFileById(idFile).subscribe(
      (response) => {
        this.httpFileService.getMetadataById(idFile).subscribe((metaData: any) => {
