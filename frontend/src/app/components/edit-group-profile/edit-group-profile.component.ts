...
export class EditGroupProfileComponent implements OnInit {
  ...
  ngOnInit() {
    this.updateGroupForm = this.formBuilder.group({
      ...
      shortDescription: ['', [Validators.minLength(3), Validators.maxLength(244)]],
      longDescription: ['', [Validators.minLength(10), Validators.maxLength(15000)]],
    });
  ...
  get f() { return this.updateGroupForm.controls; }
  ...
  updateGroup() {
    ...
    this.httpGroupService.update(this.group.id, this.updateGroupForm.value).subscribe((updatedGroup: any) => {
    ...
    if (this.coverEventReceived && this.bannerEventReceived) {
      this.httpGroupService.setBanner(this.group.id, this.idOfMetadataBanner).subscribe((res2: any) => {
        console.log('response after trying to update the banner' + res2);
          this.bannerEventReceived = false;
        this.httpGroupService.setCover(this.group.id, this.idOfMetadataCover).subscribe((res3: any) => {
      ...
      if (this.coverEventReceived) {
        this.httpGroupService.setCover(this.group.id, this.idOfMetadataCover).subscribe((res2: any) => {
          this.coverEventReceived = false;
          console.log('response after trying to update the cover' + res2);
        });
