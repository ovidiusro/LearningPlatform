...
export class UploadFileComponent implements OnInit {
  ...
  constructor(
    private fileService: HttpFilesService,
  ...
  back() {
    if (this.metadataCreated && !this.fileUploaded) {
      this.fileService.deleteMetadata(this.idOfMetadata).subscribe(
        (res) => {
  ...
  createFileMetadata() {
    ...
    this.fileService
      .createMetadata(this.f.name.value, this.f.summary.value)
      .subscribe(
        (res: any) => {
          console.log(res);
          ...
          this.uploader = new FileUploader({
            url: this.url + "/" + this.idOfMetadata,
            itemAlias: "file",
          });
