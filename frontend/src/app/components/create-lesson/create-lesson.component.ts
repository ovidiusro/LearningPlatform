...
export class CreateLessonComponent implements OnInit {
  ...
  // receiveEventForLessonFiles($event) {
  ...
  ngOnInit() {
    ...
    this.lessonForm = this.formBuilder.group({
      ...
      shortDescription: ['', [Validators.required, Validators.minLength(3), Validators.maxLength(1000)]],
  ...
  registerLesson() {
    ...
    this.httpLesssonService.registerLesson(this.lessonForm.value).subscribe((newLesson: any) => {
      this.httpGroupService.addLesson(this.idGroup, newLesson.id).subscribe((res: any) => {});
