...
export class GroupOverviewComponent implements OnInit {
  ...
  constructor(private groupDetail: GroupDetailsComponent,
  ...
  checkIfRegistered() {
    ...
    this.httpGroupService.getFolowers(this.group.id).subscribe((res2: any) => {
      const list: string[] = [];
  ...
  unsubscribing() {
    this.httpGroupService.removeFollower(this.group.id, this.principal.id).subscribe((res: any) => { this.subscribed = false; });
  }
  subscribing() {
    if (this.isLogged === true) {
    this.httpGroupService.addFollower(this.group.id, this.principal.id).subscribe((res: any) => { this.subscribed = true; });
    } else {
