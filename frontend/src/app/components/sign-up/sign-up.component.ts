...
export class SignUpComponent implements OnInit {
  ...
  signUp() {
    ...
    this.httpAccountService.getUserByUsername(this.f.username.value).subscribe(
      (response: any) => {
      ...
      (err) => {
        if (err.status === 404) {
          this.httpAccountService.getUserByEmail(this.f.email.value).subscribe(
            ...
            (err2) => {
              if (err2.status === 404) {
                this.httpAccountService.signUp(this.signUpForm.value).subscribe(
                  (data: any) => {
                    this.msg =
                      "Your account has been successfuly created. Please sign in.";
                  ...
                  (err3: HttpErrorResponse) => {
                    ...
                    this.errorMsg =
                      "Sorry, an error occured when creating your accont.";
                    this.msg = "";
                  }
                );
              }
