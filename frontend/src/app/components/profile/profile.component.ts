...
export class ProfileComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.updateProfileForm = this.formBuilder.group({
      ...
      confirmPassword: ['', [Validators.maxLength(50), Validators.minLength(6)]],
      email: ['', [Validators.maxLength(50), Validators.minLength(3), Validators.email]],
  ...
  registerFile() {
    ...
    if (this.avatarEventReceived) {
      this.httpAccountService.setAvatar(this.user.id, this.idMetadataAvatar).subscribe((res: any) => {
  ...
  updateProfile() {
    ...
    if (usernameUpdate && emailUpdate) {
      this.httpAccountService.getUserByUsername(this.f.username.value).subscribe((response: any) => {
        ...
        if (err.status === 404) {
          this.httpAccountService.getUserByEmail(this.f.email.value).subscribe((response: any) => {
  ...
  updateAccount() {
    this.httpAccountService.update(this.user.id, this.updateProfileForm.value).subscribe((res: any) => {
